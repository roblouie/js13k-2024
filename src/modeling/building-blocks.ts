...
export class SegmentedWall extends MoldableCubeGeometry {
  totalWidth = 0;
  constructor(segmentWidths: number[], segmentHeight: number, topSegments: number[], bottomSegments: number[], startingX = 0,startingY = 0, depth = 1, segmentTop = false, segmentBottom = false) {
    let runningSide = 0;
    let runningLeft = 0;
...
export function createBox(frontWall: SegmentedWall, backWall: SegmentedWall, leftWall: SegmentedWall, rightWall: SegmentedWall) {
  return createHallway(frontWall, backWall, (leftWall.totalWidth + 1) / 2)
    .merge(createHallway(leftWall, rightWall, (frontWall.totalWidth - 1) / 2).rotate_(0, Math.PI / 2)).computeNormals().done_();
}
