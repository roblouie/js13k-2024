...
precision highp float;
in vec4 h;in vec2 o;in float n;in vec3 u;in mat4 s;in vec4 t;in vec3 g;uniform vec2 z;uniform vec4 A;uniform vec3 v;uniform mediump sampler2DArray C;uniform mediump sampler2DShadow D;uniform mediump samplerCube K;out vec4 J;vec3 I=vec3(-1,1.5,-1);float H=.2f,d=1.f;vec2 F[5]=vec2[](vec2(0),vec2(-1,0),vec2(1,0),vec2(0,1),vec2(0,-1));float E=1.,B=4200.;float p(float v,float m,float f,float i){return 1./(v*v*m+v*f+i);}void main(){vec3 m=g-v,l=v-g;float f=length(m);vec3 i=normalize(m),d=normalize(l);float e=texture(K,i).x*40.,B=0.;B=e+.015<f?0.:1.;vec3 h=normalize(mat3(s)*u),E=length(A)>0.?A.xyz:vec3(1);float t=max(0.,dot(d,normalize(h))),H=p(f,.005,.001,.4);vec4 D=vec4(E.xyz*clamp(t*H*B,.05,1.),1);J=n<0.?D:texture(C,vec3(o*z,n))*D;}`;
