...
const lightViewLookPoint = new EnhancedDOMPoint();
export function render(camera: Camera, scene: Scene) {
  ...
  cubeMap.sides.forEach((side, i) => {
    ...
    const lightView = createLookAt(lightInfo.pointLightPosition, lightViewLookPoint.addVectors(lightInfo.pointLightPosition, side.target), side.up);
